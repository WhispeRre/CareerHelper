...
**careerhelper** 是一款 AI 驱动的智能简历生成器，用户可通过拖拉拽方式可视化创建简历，通过 AI 对话优化简历内容，支持 PDF 导出、多份简历管理、多语言界面（中文/英文）。基于 Next.js 全栈框架开发。
...
if (config.db.type === 'postgresql') {
  ...
  adapter = new SQLiteAdapter(process.env.SQLITE_PATH || './data/careerhelper.db');
}
...
DATABASE_URL=postgresql://user:password@localhost:5432/careerhelper

# SQLite（DB_TYPE=sqlite 时使用）
SQLITE_PATH=./data/careerhelper.db
