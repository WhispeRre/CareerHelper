...
[![Docker](https://img.shields.io/badge/Docker-Ready-2496ed)](https://hub.docker.com/r/whisperre/careerhelper)
...
docker run -d -p 3000:3000 \
  ...
  -e DATABASE_URL=postgresql://user:pass@host:5432/careerhelper \
  whisperre/careerhelper:latest
```
...
| `SQLITE_PATH` | No | `./data/careerhelper.db` | SQLite database file path |
...
| `APP_NAME` | No | `careerhelper` | Application display name |
...
careerhelper includes **50 professionally designed resume templates** covering a wide range of styles and industries:
...
careerhelper does not require server-side AI API keys. Each user configures their own AI provider (OpenAI, Anthropic, or custom endpoint), API key, and model in **Settings > AI** within the app. API keys are stored in the browser's local storage and are never sent to the server for storage.
...
When `AUTH_ENABLED=false` (default), careerhelper uses browser fingerprinting via FingerprintJS. A unique fingerprint ID is generated for each browser and used as the user identifier. No login screen is shown — users can start building resumes immediately.
